...
<head>
    <title>Code coverage report for src/malta.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../index.html">All files</a> / <a href="index.html">src</a> malta.js</h1>
        <div class='clearfix'>
            
            <div class='fl pad1y space-right2'>
                <span class="strong">89.41% </span>
                <span class="quiet">Statements</span>
                <span class='fraction'>456/510</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>148/210</span>
            </div>
            ...
            <div class='fl pad1y space-right2'>
                ...
                <span class='fraction'>437/484</span>
            </div>
...
<a name='L1381'></a><a href='#L1381'>1381</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
const fs = require('fs'),
    ...
    TAB = '\t',
    UNDEFINED = 'undefined',
    getCommentFn = (pre, post) =&gt; <span class="fstat-no" title="function not covered" >cn</span>t =&gt; <span class="cstat-no" title="statement not covered" >pre + cnt + post,</span>
    objMultiKey = o =&gt; {
        const ret = {};
        let i, j, jl, ks;
        for (i in o) {
 ...
 * @type {String}
 */
Malta.name = 'name' in packageInfo ? packageInfo.name : <span class="branch-1 cbranch-no" title="branch not covered" >'Malta';</span>
&nbsp;
/**
 ...
 * @type {String}
 */
Malta.version = 'version' in packageInfo ? packageInfo.version : <span class="branch-1 cbranch-no" title="branch not covered" >'x.y.z';</span>
&nbsp;
/**
 ...
 * @type {String}
 */
Malta.author = 'version' in packageInfo ? packageInfo.version : <span class="branch-1 cbranch-no" title="branch not covered" >'x.y.z';</span>
&nbsp;
/**
...
&nbsp;
    command.stdout.on('data', <span class="fstat-no" title="function not covered" >fu</span>nction (data) {
<span class="cstat-no" title="statement not covered" >        Malta.log_debug(`${data}`);</span>
    });
    command.on('close', function (code) {
        Malta.executeCheck += ~~code;
        if (code) process.exit(1);
        <span class="missing-if-branch" title="else path not taken" >E</span>if (typeof then !== UNDEFINED) then();
    });
    command.on('error', <span class="fstat-no" title="function not covered" >fu</span>nction (code, err) {
<span class="cstat-no" title="statement not covered" >        Malta.executeCheck += ~~code;</span>
<span class="cstat-no" title="statement not covered" >        Malta.log_debug(`&gt; \`${exe}\` child process exited with code ${code}`);</span>
...
Malta.badargs = (tpl, dst) =&gt; {
    const msg = [
        'ERR : It looks like',
        NL,
        (tpl ? tpl + NL : <span class="branch-1 cbranch-no" title="branch not covered" >'')</span>,
        (dst ? <span class="branch-0 cbranch-no" title="branch not covered" >dst + NL </span>: ''),
        'can`t be found!',
        NL,
        '... check, fix and rerun',
        NL
...
&nbsp;
    ...
    <span class="missing-if-branch" title="else path not taken" >E</span>if (errs.length) {
        Malta.stop(() =&gt; {
...
Malta.stop = what =&gt; {
    ...
    fs.unlink(Malta.printfile, <span class="fstat-no" title="function not covered" >()</span> =&gt; { });
    Malta.running = false;
    process.exit();
};
&nbsp;
...
<span class="cstat-no" title="statement not covered" >            Malta.log_info('Looks like a json configuration file is incorrect');</span>
<span class="cstat-no" title="statement not covered" >            Malta.log_err(err);</span>
<span class="cstat-no" title="statement not covered" >            Malta.stop();</span>
        }
    }
    if (demon) {
        for (i in ret) {
...
Malta.prototype.doErr = <span class="fstat-no" title="function not covered" >(e</span>rr, obj, pluginName) =&gt; {
<span class="cstat-no" title="statement not covered" >    Malta.log_debug(`[ERROR on ${obj.name} using ${pluginName}] :`.red());</span>
};
&nbsp;
/**
...
Malta.log_debug = Malta.prototype.log_debug = function (msg) {
    ...
    msg = (this.proc ? <span class="branch-0 cbranch-no" title="branch not covered" >`${this.proc} ` </span>: '') + msg;
    console.log(msg);
    return msg;
};
&nbsp;
...
Malta.log_dir = Malta.prototype.log_dir = function (msg) {
    ...
    msg = (this.proc ? <span class="branch-0 cbranch-no" title="branch not covered" >`${this.proc} ` </span>: '') + JSON.stringify(msg);
    console.log(msg);
    return msg;
};
&nbsp;
...
Malta.log = Malta.prototype.log = function (msg) {
    <span class="missing-if-branch" title="else path not taken" >E</span>if (Malta.verbose &gt; 0) {
        msg = (this.proc ? <span class="branch-0 cbranch-no" title="branch not covered" >`${this.proc} ` </span>: '') + '[LOG]: '.yellow() + msg.white();
        process.env.NODE_ENV !== 'test' &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >Malta.log_debug(msg);</span>
        return msg;
    }
};
&nbsp;
...
&nbsp;
    <span class="missing-if-branch" title="else path not taken" >E</span>if (this.justCopy === false) {
...
<span class="cstat-no" title="statement not covered" >            Malta.log_debug(`Malta error writing file '${self.outName}' error: `);</span>
<span class="cstat-no" title="statement not covered" >            Malta.log_dir(err);</span>
<span class="cstat-no" title="statement not covered" >            Malta.stop();</span>
        }
&nbsp;
        const d = self.date(),
...
&nbsp;
    ...
    <span class="missing-if-branch" title="if path not taken" >I</span>if (l &gt; 10 * utils.uniquearr(this.queue).length) {
<span class="cstat-no" title="statement not covered" >        this.log_info(`OOOOUCH: it seems like running a circular file inclusion'${NL}${TAB}try to look at the following inclusion queue to spot it quickly:${NL}this.queue`.red());</span>
    }
&nbsp;
    // look for too many files limit
    this.involvedFiles &gt; this.MAX_INVOLVED &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this.log_err(`OUCH: it seems like trying to involve too many files: ${l}`.white());</span>
};
&nbsp;
/**
 * [check description]
...
&nbsp;
    ...
    //
    <span class="missing-if-branch" title="if path not taken" >I</span>if (a.length &lt; 2) <span class="cstat-no" title="statement not covered" >Malta.log_help();</span>
&nbsp;
    // demon ?
    tmp = a[0].match(/#(.*)/);
    if (tmp) {
...
&nbsp;
    !(fs.existsSync(this.tplPath)) &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >badArgs.push(this.tplPath);</span>
...
&nbsp;
    ...
    &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >this.log_err('Output and template directories coincide. Malta won`t overwrite your template'.red());</span>
&nbsp;
...
Malta.prototype.getSize = aPath =&gt; {
    ...
    return kbyted &lt; 1 ? `${byted.toFixed(2)} B` : <span class="branch-1 cbranch-no" title="branch not covered" >`${kbyted.toFixed(2)} KB`;</span>
};
&nbsp;
/**
 * [hasVars description]
...
&nbsp;
    if (tmp &amp;&amp; tmp.length &amp;&amp; tmp[1]) {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if ('verbose' in self.options) Malta.verbose = parseInt(self.options.verbose, 10);
        <span class="missing-if-branch" title="if path not taken" >I</span>if ('watchInterval' in self.options) <span class="cstat-no" title="statement not covered" >self.watchInterval = parseInt(self.options.watchInterval, 10);</span>
        if ('showPath' in self.options) self.showPath = !!(self.options.showPath);
        <span class="missing-if-branch" title="if path not taken" >I</span>if ('notifyBuild' in self.options) <span class="cstat-no" title="statement not covered" >self.notifyBuild = !!(self.options.notifyBuild);</span>
        <span class="missing-if-branch" title="if path not taken" >I</span>if ('justCopy' in self.options) <span class="cstat-no" title="statement not covered" >self.justCopy = !!(self.options.justCopy);</span>
...
&nbsp;
    for (null; i &lt; l; i++) {
        (function (j) {
            parts = p[j].match(/([^[]*)(\[(.*)\])?/);
            self.pluginManager.add(parts[1], utils.jsonFromStr(parts[3]) || <span class="branch-1 cbranch-no" title="branch not covered" >false)</span>;
 ...
 */
Malta.prototype.listen = <span class="fstat-no" title="function not covered" >fu</span>nction (fpath) {
    // listen to changes
<span class="cstat-no" title="statement not covered" >    if (!(fpath in this.files)) {</span>
<span class="cstat-no" title="statement not covered" >        this.files[fpath] = utils.createEntry(fpath);</span>
    }
};
&nbsp;
...
&nbsp;
    ...
    if (fs.existsSync(self.varPath)) {
        try {
            ...
            <span class="missing-if-branch" title="else path not taken" >E</span>if (utils.validateJson(tmp)) {
                tmp = JSON.parse(tmp);
                try {
                    self.vars = utils.solveJson(tmp);
                } catch (e) {
<span class="cstat-no" title="statement not covered" >                    e.stop &amp;&amp; Malta.stop(e.message);</span>
                }
                self.log_debug('Loaded vars file '.yellow() + NL + self.varPath);
            } else {
<span class="cstat-no" title="statement not covered" >                Malta.log_debug(`${self.varPath} not valid`.red());</span>
<span class="cstat-no" title="statement not covered" >                self.vars = {};</span>
            }
        } catch (e) {
<span class="cstat-no" title="statement not covered" >            self.vars = {};</span>
        }
...
&nbsp;
    msg = [
        msg ? (msg + NL) : <span class="branch-1 cbranch-no" title="branch not covered" >'',</span>
        'build #',
        this.buildnumber,
        ' in ',
        `${end - start}`.white(),
 ...
 * { function_description }
 */
Malta.prototype.delete_result = <span class="fstat-no" title="function not covered" >fu</span>nction () {
<span class="cstat-no" title="statement not covered" >    Malta.log_debug(this.outName);</span>
};
&nbsp;
/**
 * [parse description]
        ...
        if (els) {
            ...
            for (let i = 0, l = els.length; i &lt; l; i++) {
                const p = els[i].match(new RegExp(self.reg[self.placeholderMode].files)),
                    f = p[2],
                    fname = f.match(/^\//)
                        ? <span class="branch-0 cbranch-no" title="branch not covered" >execPath + f</span>
                        : self.baseDir + DS + f;
                let tmp;
&nbsp;
                <span class="missing-if-branch" title="else path not taken" >E</span>if (f) {
                    <span class="missing-if-branch" title="else path not taken" >E</span>if (self.queue[self.queue.length - 1] !== fname) self.queue.push(fname);
&nbsp;
...
&nbsp;
                    ...
                    <span class="missing-if-branch" title="else path not taken" >E</span>if (tmp) {
...
<span class="cstat-no" title="statement not covered" >            Malta.log_debug('Malta microtemplating error evaluating code: '.red());</span>
<span class="cstat-no" title="statement not covered" >            Malta.log_debug(ev.join(NL));</span>
<span class="cstat-no" title="statement not covered" >            Malta.stop(e.message);</span>
        }
        return r.join(NL);
    }
    return cnt;
...
&nbsp;
    return tpl.replace(new RegExp(self.reg[self.placeholderMode].files, 'g'), (str, $1, $2, $3, $4) =&gt; {
        ...
        const ext = utils.getFileExtension($2),
            fname = $2.match(/^\//)
                ? <span class="branch-0 cbranch-no" title="branch not covered" >execPath + $2</span>
                : self.baseDir + DS + $2;
&nbsp;
        // file not found
        //
        <span class="missing-if-branch" title="if path not taken" >I</span>if (!fs.existsSync(fname)) {
            // warn the user through console
            //
<span class="cstat-no" title="statement not covered" >            self.log_info([</span>
                '[WARNING]'.red(),
                'missing file',
                fname
            ].join(' '));
&nbsp;
            ...
            //
<span class="cstat-no" title="statement not covered" >            if (ext in self.comments) {</span>
<span class="cstat-no" title="statement not covered" >                return self.comments[ext](` ### ${$2} ### `);</span>
            }
&nbsp;
            // remove it
...
&nbsp;
        ...
        tmp = tmp.replace(
            ...
            function (str, $1, $2) { return $2 || <span class="branch-1 cbranch-no" title="branch not covered" >str;</span> }
        );
&nbsp;
        // maybe add path tip in build just before file inclusion
        //
        <span class="missing-if-branch" title="if path not taken" >I</span>if (self.showPath &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >ext in self.comments)</span> {
<span class="cstat-no" title="statement not covered" >            tmp = self.comments[ext](`[${Malta.name}] ${$2}`) + tmp;</span>
        }
&nbsp;
...
&nbsp;
        ...
        return ext !== 'xml'
            ? $1 + tmp.replace(/\n/g, NL + $1)
            : <span class="branch-1 cbranch-no" title="branch not covered" >tmp;</span>
    });
};
&nbsp;
/**
...
Malta.prototype.replace_vars = function (tpl) {
    const self = this,
        rep = <span class="fstat-no" title="function not covered" >va</span>l =&gt; {
<span class="cstat-no" title="statement not covered" >            switch (self.placeholderMode) {</span>
                case 'dolla':
<span class="cstat-no" title="statement not covered" >                    return `$${val}$`;</span>
                case 'func':
<span class="cstat-no" title="statement not covered" >                    return `maltaV('${val}')`;</span>
                default: <span class="cstat-no" title="statement not covered" >return val;</span>
            }
        };
&nbsp;
...
&nbsp;
        if (typeof t === 'object') {
            t = JSON.stringify(t);
            <span class="missing-if-branch" title="else path not taken" >E</span>if (typeof t !== UNDEFINED) {
                return t;
            }
        }
        if (isPackageVar) {
        ...
        return typeof t !== UNDEFINED ? t : <span class="branch-1 cbranch-no" title="branch not covered" >rep($1);</span>
    });
};
&nbsp;
/**
...
<span class="cstat-no" title="statement not covered" >        fs.writeFileSync(fname, '{}');</span>
    }
};
&nbsp;
/**
...
&nbsp;
            for (f in self.files) {
                ...
                //
                <span class="missing-if-branch" title="if path not taken" >I</span>if (!fs.existsSync(f)) {
<span class="cstat-no" title="statement not covered" >                    self.shut();</span>
<span class="cstat-no" title="statement not covered" >                    Malta.log_debug('REMOVED '.yellow() + f + NL);</span>
&nbsp;
                    // something changed ?
                    //
                } else <span class="missing-if-branch" title="if path not taken" >I</span>if (self.files[f].time &lt; utils.getFileTime(f)) {
<span class="cstat-no" title="statement not covered" >                    d = self.date();</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >                    self.log_info(`[${'MODIFIED'.yellow()} @ ${d.getHours()}:${d.getMinutes()}:${d.getSeconds()}] ${f.replace(self.baseDir + DS, '').underline()}`);</span>
                    // renew entry
                    //
<span class="cstat-no" title="statement not covered" >                    self.files[f] = utils.createEntry(f);</span>
&nbsp;
                    // active flag rebuild
                    //
<span class="cstat-no" title="statement not covered" >                    self.doBuild = true;</span>
&nbsp;
                    // if it`s vars .json reload it
                    //
<span class="cstat-no" title="statement not covered" >                    if (f === self.varPath) {</span>
                        // update vars
<span class="cstat-no" title="statement not covered" >                        varsContent = utils.cleanJson(fs.readFileSync(self.varPath, { encoding: 'UTF8' }));</span>
<span class="cstat-no" title="statement not covered" >                        if (utils.validateJson(varsContent)) {</span>
<span class="cstat-no" title="statement not covered" >                            varsContent = JSON.parse(varsContent);</span>
<span class="cstat-no" title="statement not covered" >                            try {</span>
<span class="cstat-no" title="statement not covered" >                                self.vars = utils.solveJson(varsContent);</span>
                            } catch (e) {
<span class="cstat-no" title="statement not covered" >                                e.stop &amp;&amp; Malta.stop(e.message);</span>
                            }
                        } else {
...
&nbsp;
<span class="cstat-no" title="statement not covered" >                    self.parse(f);</span>
                }
            }
            <span class="missing-if-branch" title="if path not taken" >I</span>if (self.doBuild) <span class="cstat-no" title="statement not covered" >self.build();</span>
        };
    let d, f, varsContent;
&nbsp;
...
&nbsp;
    this.watch_TI = setInterval(function () {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (!self.doBuild) watch();
    }, this.watchInterval);
&nbsp;
    // chain
    //
...
//
process.on('SIGINT', <span class="fstat-no" title="function not covered" >()</span> =&gt; { <span class="cstat-no" title="statement not covered" >Malta.stop('SIGINT'); </span>});
process.on('exit', () =&gt; {
    // if programmatic
    if (!global.BIN_MODE) {
        Malta.verbose = 0;
...
&nbsp;</pre></td></tr></table></pre>
            ...
            <div class='footer quiet pad2 space-top1 center small'>
                ...
                at Sun Nov 21 2021 10:48:57 GMT+0100 (Central European Standard Time)
            </div>
        </div>
